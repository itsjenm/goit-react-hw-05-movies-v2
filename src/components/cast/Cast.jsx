    ...
    <div>
       {cast && (
        ...
        <ul className={Styled.cast_list}>
            {cast.map((person) => //brackets not needed in order to render correctly
               <li key={person.id} className={Styled.cast_profile} >
                    <a href={`https://www.google.com/search?q=${person.original_name}`} target='_blank' rel='noreferrer'>
                        <div>
                            {person.profile_path ? (
                                <img src={`https://image.tmdb.org/t/p/original${person.profile_path}`} className={Styled.cast_image} alt={person.original_name}/>
                            ) : (
